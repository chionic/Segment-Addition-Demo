...
var sliceRadians = [0.58643, 0.39095, 0.19548] #0.58, 0.195
...
func _ready():
	...
	scene[0] = preload("res://StitchDiskScenes/ForestScenes/forest_3.tscn")
	scene[1] = preload("res://StitchDiskScenes/ForestScenes/forest_2.tscn")
	scene[2] = preload("res://StitchDiskScenes/ForestScenes/forest_1.tscn")
...
func addSlicesTest(direction):
	...
	if((direction == -1 or direction == 1) and firstTurn > 1 and lastRotationCleared): # and firstTurn <= 4):
		if(skipNext):
			...
			if(startEnd[0] != startEnd[1]):
				...
				moveOver(totalMove,startEnd[2], direction) #first input: startEnd[1]-direction
	...
	if(direction == 0 and lastDir != 0):
		...
		print("GOAL SLICE IS: ", goalSlice, "________________________________________________")
		var xy = user.check_if_facing(goalSlice,0.9) #0.8
		...
		if(xy):
			...
			var averageTurnSpeed = (PI + additionalTurnAmountA)/ (turnTime/1000)
...
func addSliceToList(sliceType, index, lastSlice, dir):
	...
	if(dir == 1):
		instancedScene.rotate_y(lastSlice.returnNodeRadians())
		print("Last slice index: ", lastSlice, " radians: ", lastSlice.returnNodeRadians())
	else:
		instancedScene.rotate_y(-lastSlice.returnNodeRadians())
		print("Last slice index: ", lastSlice, " radians: ", -lastSlice.returnNodeRadians())
	instancedScene.transform.origin = Vector3(0,9,0) #-0.502
...
func removeSliceFromList():
	...
	while(extraSlices.size() > count):
		...
		print("Child slice: ", slice.get_child(1), " count: ", count)
		#remove the slice if the player can't currently see it
		if(!slice.get_child(1).is_on_screen()):
			print("Removed ", slice.get_child(1))
			extraSlices.erase(slice)
			slice.queue_free()
		#otherwise skip that slice and move onto the next one in the list
		else:
			print("slice: ", slice.get_child(1), "before: ", slice.get_child(1).pastTurn)
			slice.get_child(1).pastTurn = true
			print("turned true: ", slice.get_child(1).pastTurn)
			extraSlices.erase(slice)
			#count+=1
...
func moveOverAddRelative(index, addedSliceAmount, stopMoving, dir):
	...
	if(index != stopMoving):
		...
		while(currSlice != stopMoving2 && currSlice != -1 && count <= 6):
			print(currSlice, " stopMoving slice: ", stopMoving)
			if(dir == -1):
			...
			sliceList[currSlice].rotate_y((sliceList[prevSlice].returnNodeRadians() + sliceRadians[2]) * dir)
			#sliceList[currSlice].transform.origin.y += 0.2
			prevSlice = currSlice
			currSlice = arrayRotation(index, currSlice, dir)
			count+=1
		#addition to hopefully stop slices piling on top of each other when starting moveOver method
		sliceList[currSlice].transform.basis = sliceList[prevSlice].transform.basis
		sliceList[currSlice].rotate_y((sliceList[prevSlice].returnNodeRadians() + sliceRadians[2]) * dir)
		#sliceList[currSlice].transform.origin.y += 0.2
		additionalTurnAmountA = (sliceRadians[2] * (count))
		...
		#if(count == 8): #5
		#	return arrayRotation(prevSlice, prevSlice, dir)
		return currSlice
	return stopMoving
	...
	if(index != stoppingPoint):
		...
		while(currSlice != -1  and currSlice != stoppingPoint):
			...
			if(dir == 1):
				...
				sliceList[currSlice].rotate_y(sliceList[currSlice].returnNodeRadians()*dir)
...
func rotateSlice(currSlice, direction, lastSlice):
	...
	if(currSlice != -1 and lastSlice != -1):
		...
		push_warning("Last Slice: ", lastSlice, " currSlice: ", currSlice, " direction: ", direction, " rotation amount: ",
		sliceList[lastSlice].returnNodeRadians())
		if(direction == 1):
			sliceList[currSlice].rotate_y(sliceList[lastSlice].returnNodeRadians() * direction) #0.59 if you want slight gap for debugging
		else:
...
func findSliceUserCanSeeAlt(dir):
	...
	if(dir == 1):
		...
		if(start < end or abs(start - end) > 11):
		...
		if(end < start or abs(end - start) > 11 ):
...
func isPastBoundary(testSlice, inputSlice, dir):
	...
	if(dir == 1):
		var upperBound = (sliceList[inputSlice].transform.basis.get_euler()[1] + 0.25)
		if(upperBound > 2*PI):
			upperBound -= (2*PI)
		var lowerBound = sliceList[inputSlice].transform.basis.get_euler()[1] - 0.15
		if(lowerBound < 0):
		...
		var upperBound = (sliceList[testSlice].transform.basis.get_euler()[1] + 0.15)
		if(upperBound > 2*PI):
			upperBound -= (2*PI)
		var lowerBound = sliceList[testSlice].transform.basis.get_euler()[1] - 0.257
		if(lowerBound < 0):
			lowerBound = lowerBound + (2*PI)
